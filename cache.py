...
class cache(dict):
	...
	def __getitem__(self,key):
		...
		try:
			if hasattr(self,"read"): 
				...
				print u"Чтение c  диска  %s за %s"%(key,t.interval())
				return value
		except Exception,e:
			pass
		return None
	...
	def save(self):
		...
		if os.path.isfile(old_filename):
			#self.z = zipfile.ZipFile(old_filename,"r",zipfile.ZIP_DEFLATED)
			for k in set(self.z.namelist()) - set(self.keys()) :			#перезаписываем на диске
